...
function fetch_file_selector($dirRead,$dirWrite,$filefilter,$realPrefix) {
	...
	for ($i=0;$i<count($arDir);$i++) {
		...
		foreach (glob($arDir[$i]."/".$filefilter) as $filename) {
...
$dirRead = $_SESSION["TINYMCE_MEDIA_LIST_PATH"];
$dirWrite = $_SESSION["TINYMCE_MEDIA_LIST_URL"];

$output = '';
$delimiter = "\n";
$output .= 'var tinyMCEMediaList = new Array(';

$ar = fetch_file_selector($dirRead,$dirWrite,"*.*",$prefixMedia);
